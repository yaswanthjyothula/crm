    ...
    >
      <nav 
        className={`pointer-events-auto flex items-center justify-between w-full max-w-5xl px-4 sm:px-6 py-2.5 sm:py-3 rounded-full transition-all duration-300 ${
          scrolled 
            ? 'glass-pill shadow-xl bg-white/90 border-slate-200 backdrop-blur-xl' 
            : 'glass-pill bg-white/80 border-slate-200/80 backdrop-blur-md'
        }`}
        aria-label="Main Navigation"
      >
        {/* Brand Mark: High-End Serif PULSE SERVE Wordmark */}
        <a 
          href="#" 
          className="flex items-center gap-2 group focus:outline-none focus:ring-2 focus:ring-brand-500/50 rounded-full px-2 py-1"
